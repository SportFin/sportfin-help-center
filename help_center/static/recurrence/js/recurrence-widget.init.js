...
function initRecurrenceWidget($field) {
	if (!$field) {
		const recurrenceFields = jQuery(document).find('textarea.recurrence-widget:not([id*="__prefix__"])');
		jQuery.each(recurrenceFields, function (index, field) {
	...
	for (const mutation of mutationsList) {
		if (mutation.type === 'childList') {
			...
			if (addedNodes.length > 0) {
				const $addedRecurrenceField = jQuery(addedNodes[0]).find('.recurrence-widget');
